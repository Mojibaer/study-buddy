...
def get_all_filters(db: Session = Depends(get_db)) -> FiltersResponse:
    return {
...
def list_semesters(db: Session = Depends(get_db)) -> list[SemesterResponse]:
    return db.query(Semester).all()


@router.post("/semesters", response_model=SemesterResponse)
def create_semester(semester: SemesterCreate, db: Session = Depends(get_db)) -> SemesterResponse:
...
def list_subjects(semester_id: int | None = None, db: Session = Depends(get_db)) -> list[SubjectResponse]:
...
def create_subject(subject: SubjectCreate, db: Session = Depends(get_db)) -> SubjectResponse:
    get_semester_or_404(db, subject.semester_id)
...
def delete_semester(semester_id: int, db: Session = Depends(get_db)) -> dict[str, str]:
